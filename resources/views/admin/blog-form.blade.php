...
$body = $blog->blog_desc ?? null;
$image = $blog->images ?? null;
$cate = $blog->category_id ?? null;
@endphp
...
<div class="padding">
    ...
    <h3 style="color:#1F85EC">Add Blog</h3>
    <div class="box">
        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-3">
                    <a type="button" class="btn btn-primary text-white" href="{{ route('admin.blog')}}">
                        Blogs Lits</a>
        ...
        <div class="container table-responsive">
            <form action="{{ route('admin.form.add-blog')}}" method="POST" enctype="multipart/form-data" class="container">
                @csrf
                    ...
                    <div class="col-md-10">
                        <select name="category" class="form-control" required> 
                          ...
                          @foreach($categories as $category)
                            <option @if($cate == $category->id) selected @endif value="{{ $category->id }}">{{ $category->cate_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    <div class="col-md-2">
                        <label for="inputEmail3" class="form-control-label">Select Blog Image</label>
                    </div>
                    <div class="col-md-10">
                        <input type="file" @if(!$image) required @endif name="image" class="form-control" onchange="loadFile()" accept=".png, .jpg, .jpeg">
                        <p>
                            <img id="output" width="100" @if($image)
                                        src="{{asset('/uploads/admin/blogs')}}/{{$image}}" @endif>
                        </p>
                        @error('image')
                        <p class="alert" style="color:red">{{ $message }}</p>
                        @enderror
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <textarea type="text" rows="4" cols="50" name="blog_desc" id="body"
